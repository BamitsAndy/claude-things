...
Read `.claude/ENV.md` before beginning the tier assessment.

Assess the complexity of the implementation and propose a testing tier. Base your assessment on factors such as: number of modules and their interdependencies, presence of external services or I/O, statefulness, concurrency, error surface, and expected public API size.
...
**Moderate**
Multi-module projects with some logic complexity, limited external dependencies, or a small public API. Recommended approach: pytest with behavior-verified unit tests covering core logic. No fixtures or mocks unless clearly needed.
...
### Tier Suggestion Flow

1. Assess complexity based on the signals above.
2. **Suggest** the tier with explicit reasoning before writing any artifacts.
3. **Wait for the user to confirm or adjust** the tier.
4. Proceed to artifact production only after the tier is agreed upon.

### Output

The confirmed tier and its rationale must be recorded in both:
...
- **Testing**: <!-- What testing applies to this step. Name specific scenarios and edge cases to cover. Use "None — [reason]" if no testing applies. Do not write "write tests" — name what the tests must verify. -->
...
Test results are recorded inline under the relevant step:
```
- [x] 2 - [Title] | context: docs/plan/PLAN.md
  > TESTS: PASS (4/4) | docs/tests/step_2/
```
or on failure:
```
- [ ] 2 - [Title] | context: docs/plan/PLAN.md
  > TESTS: FAIL (2/4) | docs/tests/step_2/
```
